	...
	<div class="addEdit-block" :style='{"padding":"30px","backgroundAttachment":"fixed","background":"url(http://codegen.caihongy.cn/20220805/1b9b1a67bdd14d29bbcca744990004d8.png)","backgroundSize":"cover","backgroundPosition":"center center","backgroundRepeat":"no-repeat","height":"calc(100vh - 110px)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="内容" prop="neirong">
					<el-input
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.neirong" label="内容" prop="neirong">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#000","fontWeight":"500","display":"inline-block"}'>{{ruleForm.neirong}}</span>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="详情" prop="xiangqing">
					<editor 
						style="min-width: 200px; max-width: 600px;"
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.xiangqing" label="详情" prop="xiangqing">
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.xiangqing = this.ruleForm.xiangqing.replace(reg,'../../../springboot0hxfv/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhanneixinwen/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhanneixinwenCrossAddOrUpdateFlag = false;
						 this.parent.search();
