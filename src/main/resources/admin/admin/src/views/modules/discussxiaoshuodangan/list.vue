		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"margin":"20px 0","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"0","background":"url(http://codegen.caihongy.cn/20220726/b87f4c4f9a0d4ed194c128c66d95e8bd.png) center center/cover","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('discussxiaoshuodangan','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"0","background":"url(http://codegen.caihongy.cn/20220726/b87f4c4f9a0d4ed194c128c66d95e8bd.png) center center/cover","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('discussxiaoshuodangan','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 6px 0","outline":"none","color":"#000","borderRadius":"0","background":"url(http://codegen.caihongy.cn/20220726/b87f4c4f9a0d4ed194c128c66d95e8bd.png) center center/cover","width":"99px","fontSize":"14px","height":"32px"}' v-if=" isAuth('discussxiaoshuodangan','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>


							<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 6px 0","outline":"none","color":"#000","borderRadius":"0","background":"url(http://codegen.caihongy.cn/20220726/b87f4c4f9a0d4ed194c128c66d95e8bd.png) center center/cover","width":"99px","fontSize":"14px","height":"32px"}' v-if="isAuth('discussxiaoshuodangan','查看评论')" type="primary" size="mini" @click="disscussListHandler(scope.row.id)">查看评论</el-button>
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.$http({
        url: "discussxiaoshuodangan/page",
        method: "get",
        params: params
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.$router.push({path:'/discussdiscussxiaoshuodangan',query:{refid:id}});
    },
    // 下载
    download(file){
      window.open(`${file}`)
    ...
    deleteHandler(id) {
      ...
      this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
        ...
        this.$http({
          url: "discussxiaoshuodangan/delete",
          method: "post",
          data: ids
        }).then(({ data }) => {
          if (data && data.code === 0) {
